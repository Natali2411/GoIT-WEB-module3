...
def __unpack_archive(archive_path: Path, extract_dir: str) -> None:
    ...
    archive_folder, _ = __get_file_name_and_extension(file_obj=archive_path)
...
def __get_file_name_and_extension(file_obj: Path) -> Tuple[str, Union[str, Any]]:
    ...
    file_name: str = (
        ".".join(file_info[:-1]) if len(file_info) >= 2 else ".".join(file_info)
    )
    file_ext = file_info[-1] if len(file_info) >= 2 else ""
    return file_name, file_ext


def __get_file_info_base_on_file_type(
        file_obj: Path,
...
def __move_and_rename_file(file_obj: Path, new_folder_name: str) -> None:
    ...
    if not folder_path.exists():
        folder_path.mkdir()
    file_name, file_ext = __get_file_name_and_extension(file_obj=file_obj)
    normalized_full_file_name = __get_normalized_full_file_name(
...
def process_folder_item(entity: Path) -> None:
    """
    ...
    if entity.is_file():
        ...
        if new_folder_name != "archives":
            if new_folder_name in result_data:
                result_data[new_folder_name].append(normalized_full_file_name)
            else:
                result_data[new_folder_name] = [normalized_full_file_name]
...
def __sort_folder(folder_path: str) -> Dict[str, list]:
    ...
    p = Path(folder_path)
    subtree_iter = p.glob("**/*")
    entities = list(subtree_iter)
    with ThreadPoolExecutor(max_workers=len(entities)) as executor:
        executor.map(process_folder_item, entities)
...
def sort_folder() -> None:
    """
    Method sorts all files in the directory that is passed from the console as an
    argument.
    :return: None.
    """
    if len(sys.argv) >= 2:
